...
def read_gff3_genbank(gff_file):
    ...
    read_fh = open(gff_file, 'r')
    lookup_list = ['gene', 'mRNA', 'cds', 'exon', 'transcript']
    for i, line in enumerate(read_fh):
        ...
        if len(tmp) > 7:
            if any(item.lower() == tmp[2].lower() for item in lookup_list):
                ...
                if re.search(r'gene', tmp[2], re.I):
                    dct = process_gff_gene_line(dct, tmp)

                if re.search(r'mrna', tmp[2], re.I):
                    dct, gene_id_dct = process_gff_mrna_line(dct, tmp, gene_id_dct)

                if re.search(r'transcript', tmp[2], re.I):
                    dct, gene_id_dct = process_augustus_transcript_line(dct, tmp, gene_id_dct)
...
def read_gff3_genemark(gff_file):
    dct = dct_structure()
    gene_id_dct = dct_structure()
...
def process_gff_mrna_line(dct, tmp, gene_id_dct):
    source = tmp[0]
    attribute_dct = parse_single_feature_line(tmp[8])
    gff_attribute = GffAttribute()
    rna_id = None
...
def process_augustus_transcript_line(dct, tmp, gene_id_dct):
    attribute_dct = parse_single_feature_line(tmp[8])
    source = tmp[0]
    if not attribute_dct:
        arr = tmp[8].split('.')
