...
def process_gff_gene_data(gal_id, gal_fh, org_info, scaffold, gene_id, gene_dct, scaffold_na_sequence_id):
    ...
    today = bu.get_date()
    db_table.na_sequence_imp_gene(gal_fh, gal_id.NaSequenceId, org_info, scaffold, scaffold_na_sequence_id, gene_data,
                                  today)
    data_type = 'gene'
    db_table.na_feature_imp(gal_fh, gal_id.NaFeatureId, gal_id.NaSequenceId, data_type, gene_name, "NULL")

    db_table.na_location(gal_fh, gal_id.na_location_Id, gal_id.NaFeatureId, gene_data.gene_start, gene_data.gene_end,
                         gene_data.strand)
    ...
    if 'mrna' in gene_dct:
        for rna_id, rna_dct in gene_dct['mrna'].items():
            data_type = 'mRNA'
            db_table.na_feature_imp(gal_fh, gal_id.NaFeatureId, gal_id.NaSequenceId, data_type, rna_id,
                                    gene_na_feature_id)

            rna_data = RnaInfo(rna_dct)

            db_table.na_location(gal_fh, gal_id.na_location_Id, gal_id.NaFeatureId, rna_data.start, rna_data.end,
                                 gene_data.strand)
            ...
            db_table.gene_instance(gal_fh, gal_id.GeneInstanceId, gal_id.NaFeatureId, annotation, today)

            if 'protein_sequence' in rna_dct:
                protein_sequence = rna_dct['protein_sequence']
            else:
                protein_sequence = ""
            db_table.protein(gal_fh, gal_id.ProteinId, gene_name, annotation, gal_id.GeneInstanceId, protein_sequence)
...
def process_other_rna_data(gal_fh, gal_id, rna_id, rna_dct, data_type, gene_na_feature_id, gene_data):
    rna_data = RnaInfo(rna_dct)
    db_table.na_feature_imp_rna(gal_fh, gal_id.NaFeatureId, gal_id.NaSequenceId, data_type, rna_id,
                                gene_na_feature_id)
    db_table.na_location(gal_fh, gal_id.na_location_Id, gal_id.NaFeatureId, rna_data.start,
                         rna_data.end, gene_data.strand)
    gal_id.NaFeatureId += 1
    gal_id.na_location_Id += 1
    ...
    if location in feature_dct:
        for i in feature_dct[location]:
            if len(i) == 1:
                ...
                feature_start = i[0]
                feature_end = i[1]
            db_table.na_feature_imp(gal_fh, gal_id.NaFeatureId, gal_id.NaSequenceId, feature_name, rna_id,
                                    rna_na_feature_id)
            db_table.na_location(gal_fh, gal_id.na_location_Id, gal_id.NaFeatureId, feature_start, feature_end,
                                 gene_strand)
            gal_id.NaFeatureId += 1
            gal_id.na_location_Id += 1
...
def process_repeat_data(gal_id, gal_fh, feature, feature_dct, scaffold_na_sequence_id):
    ...
    if location in feature_dct:
        for i in feature_dct[location]:
            ...
            db_table.na_feature_imp(gal_fh, gal_id.NaFeatureId, scaffold_na_sequence_id, data_type, data_type, "NULL")
            db_table.na_location(gal_fh, gal_id.na_location_Id, gal_id.NaFeatureId, feature_start, feature_end, strand)
