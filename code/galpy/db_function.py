...
class Database:
    def __init__(self, host, user, password, db, infile):
        ...
        if self.infile == 1:
            ...
            self.db_param = 0
        try:
            self.connection = pymysql.connect(self.host, self.user, self.password, self.db, local_infile=self.db_param)
            self.cursor = self.connection.cursor()
        except pymysql.Error as e:
            print("\t\tMySQL::Error in database connection. %s" % str(e))
            sys.exit(0)
